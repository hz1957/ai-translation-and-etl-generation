...
# ETL JSON Request Format Models (Request Format)
class FieldDefinition(BaseModel):
    """字段定义模型"""
    fieldLabel: str = Field(..., description="字段标签")
    fieldName: str = Field(..., description="字段名称")
    fieldType: str = Field(..., description="字段类型")

class TableRecord(BaseModel):
    """表格记录模型"""
    pass  # Dynamic fields will be added at runtime

class TableDefinition(BaseModel):
    """表格定义模型"""
    dsId: str = Field(..., description="数据集ID")
    fieldList: List[FieldDefinition] = Field(..., description="字段列表")
    recordList: List[Dict[str, Any]] = Field(default=[], description="记录列表")
    tableNameCn: str = Field(..., description="中文表名")
    tableNameEn: str = Field(..., description="英文表名")

class ETLOutputConfig(BaseModel):
    """ETL输出配置模型"""
    name: str = Field(..., description="输出名称")
    outputDsDesc: str = Field(default="", description="输出数据集描述")
    outputDsId: str = Field(..., description="输出数据集ID")
    outputDsName: str = Field(..., description="输出数据集名称")
    parentDirId: str = Field(..., description="父目录ID")
    type: str = Field(default="OUTPUT_DATASET", description="类型")

class ETLLibConfig(BaseModel):
    """ETL库配置模型"""
    etlNum: int = Field(default=1, description="ETL编号")
    name: str = Field(..., description="名称")
    remark: str = Field(default="", description="备注")
    type: str = Field(default="1", description="类型")

class ETLRequest(BaseModel):
    """ETL请求格式模型 (Request - with dataFlowId, etlLib, output, tableList)"""
    dataFlowId: str = Field(..., description="数据流ID")
    etlLib: ETLLibConfig = Field(..., description="ETL库配置")
    output: ETLOutputConfig = Field(..., description="输出配置")
    tableList: List[TableDefinition] = Field(..., description="表格列表")
    remark: Optional[str] = Field(default="任务提示词", description="备注")

    class Config:
        json_schema_extra = {
            "example": {
                "dataFlowId": "d00dde84a85ff487bab0c7f4",
                "etlLib": {
                    "etlNum": 1,
                    "name": "Test1",
                    "remark": "",
                ...
                "output": {
                    "name": "Test1",
                    "outputDsDesc": "",
                    "outputDsId": "j2e5e1264f5e246eb882d35a",
                    "outputDsName": "Test1",
                    "parentDirId": "t166bd7ca1d7b464ca2f9c6c",
                    "type": "OUTPUT_DATASET"
                },
                "tableList": [
                    {
                        "dsId": "n0b3cf20caa304af1be7ece8",
                        "fieldList": [
                            {
                                "fieldLabel": "研究中心",
                                "fieldName": "SITENM",
